...
def get_daily_price_pd(usecols=[]):
    file_dir = './data/processed/merge_data.parquet'
    if len(usecols) == 0:
        df = pd.read_parquet(file_dir)
    else:
        df = pd.read_parquet(file_dir, columns=usecols)
...
def get_daily_price_ret_pd(usecols=[]):
    file_dir = './data/processed/merge_data_ret.parquet'
    if len(usecols) == 0:
        df = pd.read_parquet(file_dir)
    else:
        df = pd.read_parquet(file_dir, columns=usecols)
...
def dta_2_parquet(file_path=""):
    if not os.path.isfile(file_path) or not file_path.endswith('.dta'):
        print(f"无效的 CSV 文件路径: {file_path}")
        return
    ...
    try:
        ...
        df = df.astype({col: str for col in df.select_dtypes(include=['object']).columns})
        parquet_path = file_path.replace('.dta', '.parquet')
        df.to_parquet(parquet_path, engine='pyarrow', index=False)
        print(f"转换成功: {file_path} -> {parquet_path}")
    except Exception as e:
        print(f"转换失败: {file_path}, 错误信息: {e}")
