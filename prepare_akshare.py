...
def fetch_stock_value(code: str, re_fetch = False):
    dir_path = os.path.join(params['price_dir'], "value")
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    file_path = os.path.join(dir_path, code + ".csv")
    if re_fetch == False and os.path.exists(file_path):
        print(f"{code}.csv 已存在，跳过下载。")
        return
    df = ak.stock_value_em(symbol=code)
    # print("列名：", df.columns.tolist())
    df.index = pd.to_datetime(df['数据日期'])
    df.to_csv(file_path, index=True, encoding="utf-8-sig")
    print(f"finish saving daily price data: {code}")
    return df

def fetch_stock_value_from_list():
    df = pd.read_csv(params['data_dir'] + '/' + params['stock_list'], encoding="utf-8-sig", dtype={"Code": str})
    stock_code_list = df["Code"].tolist()
    i = 0
    for code in stock_code_list:
        i += 1
        print(f"Downloading value {code}... now: {i} / {len(stock_code_list)}")
        fetch_stock_value(code=code)
